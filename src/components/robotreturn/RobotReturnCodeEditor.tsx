...
export default function RobotReturnCodeEditor() {
  const [code, setCode] = useState(`function judgeCircle(moves: string): boolean {
  // Write your solution here
  return false;
}`);

  const [testResults, setTestResults] = useState<{ passed: boolean; input: string; expected: boolean; actual: boolean | null; error?: string }[]>([]);
  const [showResults, setShowResults] = useState(false);

  const testCases = [
    { input: 'UD', expected: true },
    { input: 'LL', expected: false },
    { input: 'UDLR', expected: true },
    { input: 'UUDDLR', expected: false },
    { input: '', expected: true },
  ];
  ...
  const runTests = () => {
    const results = testCases.map((testCase) => {
      try {
        const userFunction = new Function('moves', code + '\nreturn judgeCircle(moves);');
              ...
              >
                ...
                <div className="ml-8 space-y-2 text-sm">
                  <div>
                    ...
                    <code className="bg-white px-2 py-1 rounded border border-slate-300">"{result.input}"</code>
                  </div>
                  <div>
            ...
            <div className="mt-6 bg-gradient-to-r from-green-500 to-emerald-500 text-white p-6 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p>Semua test cases lulus. Robotmu kembali ke origin.</p>
                </div>
              </div>
            </div>
          )}
